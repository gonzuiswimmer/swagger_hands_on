...
| コンテナ名       | 役割                               | port | URL                   |
| ---------------- | ---------------------------------- | ---- | --------------------- |
| admin-mock       | 管理ポータルサイトのモックサーバー | ー | http://localhost:8180/admin-mock |
| admin-agent-mock | 顧客管理サイトのモックサーバー     | ー | http://localhost:8180/admin-agent-mock |
| order-mock       | 業務サイトのモックサーバー         | ー | http://localhost:8180/order-mock |
...
### プロキシサーバーの役割
プロキシサーバーはリクエストをまとめて受け取り、URLによって対象のモックサーバーのコンテナへリクエストを振り分けます<br>
モックサーバーコンテナはすべて4010番ポートを開いているため、内部で `該当コンテナ:4010`へ振り分けています

### モックサーバーの使い方

Vue の設定ファイル(.env ファイル?)の接続先を対象のコンテナの URL に変更し、再起動します</br>
または、API 仕様書に記載のモックサーバーへのポートを指定して、proxy 宛にリクエストを送ります</br>
API を投げるとモックサーバーへ API 通信が投げられるので、API 仕様書で記述してある example データのレスポンスが返却されます</br>
...
また、`paths`配下のファイルはプレフィックス毎に1ファイルを作成し、階層化は基本的に行わない。
